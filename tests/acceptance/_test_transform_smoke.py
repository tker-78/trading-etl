from etl.flows import transform
...
def test_transform_indicator_smoke(monkeypatch):
    ...
    monkeypatch.setattr(transform, "SMA_FLOW_DEFAULT_PARAMS", {"periods": [14], "timeframes": ["1m"]})
    monkeypatch.setattr(transform, "EMA_FLOW_DEFAULT_PARAMS", {"periods": [14], "timeframes": ["1m"]})

    monkeypatch.setattr(transform.helpers, "build_rsi_params", lambda: {"currency_pair_code": "USD/JPY"})
    monkeypatch.setattr(transform.helpers, "build_sma_params", lambda: {"currency_pair_code": "USD/JPY"})
    monkeypatch.setattr(transform.helpers, "build_ema_params", lambda: {"currency_pair_code": "USD/JPY"})

    futures = transform.indicator.fn(block_name="test-connector")

    assert len(futures) == 3
    assert len(rsi_task.calls) == 1
    assert len(sma_task.calls) == 1
    assert len(ema_task.calls) == 1
    ...
    assert sma_task.calls[0]["sma_params"]["period"] == 14
    assert sma_task.calls[0]["sma_params"]["timeframe_code"] == "1m"

    assert ema_task.calls[0]["block_name"] == "test-connector"
    assert ema_task.calls[0]["ema_params"]["period"] == 14
    assert ema_task.calls[0]["ema_params"]["timeframe_code"] == "1m"
