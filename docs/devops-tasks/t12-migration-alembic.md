...
- [x] 現行DDLの管理方法（手動SQLなど）を棚卸しできている。
- [x] マイグレーション対象DB接続情報が確定している。
- [ ] マイグレーション命名規約・運用ルール（作成/適用/レビュー）が合意されている。

## Done定義

- [x] Alembic（または同等ツール）が導入され、初期設定がリポジトリに含まれている。
- [x] 少なくとも1本のマイグレーションを作成し、`upgrade` / `downgrade` の往復が確認できる。
- [x] 新規スキーマ変更をマイグレーション経由で適用できることを確認した。
- [x] 開発者向け実行手順がドキュメント化されている。
